...
1. **INTAKE** — Scan the repository. Identify: build system, runtime version, dependencies, coupling, Dockerfile presence. Run `mvnw dependency:tree` to map the full transitive dependency graph and `mvnw versions:display-dependency-updates` to flag outdated dependencies. Produce a risk assessment table.
...
3. **GOVERNANCE** — Check every dependency and base image against the approved library registry. Flag any shadow IT (unapproved libraries). Query GitHub Dependabot security alerts using `gh api repos/{owner}/{repo}/dependabot/alerts --jq '.[] | {package: .security_advisory.summary, severity: .security_vulnerability.severity, state: .state}'` to identify known CVEs. Report PASS or FAIL.
...
6. **VALIDATE** — Run compilation, tests, dependency check, diff allowlist verification. Run `mvnw dependency:tree` again post-upgrade to confirm no new vulnerable transitive dependencies were introduced. Do NOT run the app directly (`java -cp target/classes com.demo.Main`) — the user will demo that manually. Build the Docker image (`docker build -t orders-service .`) and run it as a named container (`docker run --name orders-demo orders-service`) so it appears in Docker Desktop. If the container runs successfully, report gate results and proceed to the next stage.
